...
export const Navbar = () => {
  ...
  return (
    <nav className="z-20 fixed top-0 z-10 text-lg  w-full max-w-full px-4 bg-light dark:bg-[#13161a] dark:text-d-text shadow-md backdrop-blur-2xl backdrop-saturate-200 border-none lg:px-8 py-2">
      <div className="flex items-center justify-between top-0 ">
        <Link
          to={"/"}
          className="ml-4 dark:text-d-text text-2xl block cursor-pointer py-1.5 font-sans font-medium leading-relaxed text-inherit antialiased"
        >
          PZG
        </Link>
        <div className="flex items-center gap-4">
          <div className="hidden mr-4 lg:block">
            <ul className="flex flex-col gap-2 mt-2 mb-4 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
              <li className="block px-5  py-2 rounded-lg  antialiased  leading-normal text-blue-gray-900 dark:bg-d-card">
                <a href="/movie" className="flex items-center dark:text-d-text">
                  Movie
                </a>
              </li>
          ...
          <div className="flex items-center gap-x-3">
            {user ? (
              <>
                <span className="text-light dark:text-d-text">{user.details.username}</span>
